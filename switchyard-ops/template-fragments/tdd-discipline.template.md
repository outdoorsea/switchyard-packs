{{/*
  ...
  template-fragments/ into ONE namespace, so `{{ template "tdd-discipline" . }}`
  would resolve against gastown's copy in a city that imports both. This pack
  does not import gastown and does not require it — pack.toml requires gascity —
  and a city may import switchyard-ops alone. When `{{ template }}` names a
  template that is not defined, renderPrompt's Execute fails and returns the RAW
  template body: the worker gets an unrendered prompt with `{{ .RigRoot }}` and
  every other action still in braces, not merely a missing section. Vendoring
  keeps the fragment resolvable from this pack's own template-fragments/ (which
  gc loads via packDirs, and via the sourcePackRoot fallback even when it does
  not), so the brakeman prompt cannot render raw because of a pack nobody
  promised to import.

  Same convention as assets/scripts/worktree-setup.sh, which is a recorded copy
  of a gastown file for the same reason. Re-verify against the sha256 above when
  bumping the gastown pin; the fragment BODY below is byte-identical to upstream,
  so a re-copy is a clean diff.
*/}}
